...
if ($busca == "") {
	...
	$sql = "SELECT  id_facilitador, nome, cargo, descricao, linkedin FROM facilitadores";
	$resultado = $conexao->query($sql);
	$total_registros = $resultado->num_rows;
} else {

	// codigo que sera executado ser o usuario  estiver pesquisando
	$sql = "SELECT id_facilitador, nome, cargo, descricao, linkedin
	FROM facilitadores
...
if ($busca == '') {
	...
	// codigo que sera executado ser o usuario não estiver pesquisando
	$sql_2 = "SELECT id_facilitador, nome, cargo, descricao, linkedin FROM facilitadores 
	...
	// codigo que sera executado ser o usuario  estiver pesquisando
	$sql_2 = "SELECT id_facilitador, nome, cargo, descricao, linkedin
	FROM facilitadores
	WHERE nome LIKE '%" . $busca . "%' 
	OR cargo LIKE '%" . $busca . "%' 
	ORDER BY nome ASC LIMIT " . $limite . " OFFSET " . $deslocamento . "";
		...
		<div id="conteudo">
			...
			<div id="corpo_conteudo">
				...
				<div id="pesquisar-recipiente">
					...
					<button id="botao_pesquisar" onclick="window.location='facilitadores.php?busca=' + document.getElementById('pesquisar').value"></button>
				</div>
						...
						<tbody>
							...
							?>
									<tr>
										...
										<td>
											<button id="botao_editar" onclick="window.location='facilitadores_alterar.php?id=<?php echo $registro[0]; ?>'"></button>
