...
if ($busca == '') {
	...
	// codigo que sera executado ser o usuario não estiver pesquisando
	$sql_2 = "SELECT id_agenda, nome, categoria, local FROM agenda
			ORDER BY data_criacao ASC LIMIT " . $limite . " OFFSET " . $deslocamento . " ";
	$resultado_2 = $conexao->query($sql_2);
	$total_registros_2 = $resultado_2->num_rows;
} else {

	// codigo que sera executado ser o usuario  estiver pesquisando
	$sql_2 = "SELECT id_agenda, nome, categoria, local
	FROM agenda
	WHERE nome LIKE '%" . $busca . "%' 
	OR categoria LIKE '%" . $busca . "%' 
	ORDER BY data_criacao ASC LIMIT " . $limite . " OFFSET " . $deslocamento . "";
		...
		<div id="conteudo">
			...
			<div id="corpo_conteudo">
				...
				<div id="pesquisar-recipiente">
					...
					<button id="botao_pesquisar" onclick="window.location='agenda.php?busca=' + document.getElementById('pesquisar').value"></button>
				</div>
						...
						<tbody>
							...
							?>
									<tr>
										...
										<td>
											<button id="botao_editar" onclick="window.location='agenda_alterar.php?id=<?php echo $registro[0]; ?>'"></button>
											<button id="botao_editar" onclick="window.location='agenda_facilitador.php?id=<?php echo $registro[0]; ?>'"></button>
